...
def main():
    ...
    for document in selected_documents:
        read_doc = document
        with open(str(read_doc), "rb") as f:
            if str(read_doc).lower().endswith('.pdf'):
                pdf_reader = PdfReader(f)
                num_pages = len(pdf_reader.pages)
                images = convert_from_path(read_doc, dpi=300, first_page=1, last_page=num_pages)
                ...
                document_text += f.read().decode("utf-8").strip() + " "
