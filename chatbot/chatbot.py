...
def get_persona_dropdown():
    ...
    for persona in personas:
        persona_filename = os.path.join("personas", persona)
...
personas = {
    name: open(file, "r").read() for name, file in PERSONAS.items()
}
...
def handle_message_events(ack, body, logger):
    ...
    if "persona" in prompt.lower():
        persona_options = [
            {"text": {"type": "plain_text", "text": f":hackgpt: *{name}*", "emoji": True}, "value": text}
        ...
        response = client.chat_postMessage(
            ...
            blocks=[
                {
                    "type": "section",
                    "text": {"type": "mrkdwn", "text": "Pick a persona from the dropdown list"},
                    "accessory": {
                        "type": "static_select",
                        "placeholder": {"type": "plain_text", "text": "Personas", "emoji": True},
...
def handle_static_select_action(ack, body, logger):
    ...
    prompt_with_persona = personas[selected_option_value] + prompt
